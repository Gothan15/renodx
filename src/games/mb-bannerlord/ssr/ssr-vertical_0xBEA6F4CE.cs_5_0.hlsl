...
void main(uint3 vThreadIDInGroup : SV_GroupThreadID, uint3 vThreadID : SV_DispatchThreadID)
{
  ...
  float4 fDest;
  r0.x = cmp((uint)vThreadIDInGroup.y < 4);
  if (r0.x != 0) {
    r0.xy = (uint2)vThreadID.xy << int2(1,1);
    r0.xy = (int2)r0.xy + int2(0,-8);
    ...
    g0[vThreadIDInGroup.y].val[0/4] = r0.x;
    g0[vThreadIDInGroup.y].val[0/4+1] = r0.y;
    g0[vThreadIDInGroup.y].val[0/4+2] = r0.z;
    g0[vThreadIDInGroup.y].val[0/4+3] = r0.w;
  }
  r0.x = cmp((uint)vThreadIDInGroup.y >= 124);
  if (r0.x != 0) {
    r0.x = (int)vThreadIDInGroup.y + 8;
    r0.yz = (uint2)vThreadID.xy << int2(1,1);
    r0.yz = (int2)r0.yz + int2(0,8);
