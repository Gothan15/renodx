  ...
  out float4 o0 : SV_TARGET0)
{
  ...
  if (r0.y != 0) {
    ...
    float lightDist = r0.x;                  // Save before r0.x is overwritten
    ...
    bool validLightData = (cb0[52].w > 0.001) && (cb0[52].w < 1000.0) &&
                          ...
                          (dot(cb0[52].xyz, cb0[52].xyz) > 0.01) &&
                          (CUSTOM_RANDOM > 0);  // Skip first frame(s) before injection is ready
    
    if (CUSTOM_SUN_INTENSITY > 0 && validLightData) {
      // [A] Corona calc
      float2 centerOffset = unclampedUV - 0.5;
      float distFromCenter = length(centerOffset) * 2.0;
      sunCorona = exp(-distFromCenter * 3.0) * 1.5;
      sunCorona += exp(-distFromCenter * 8.0) * 0.5;
      
      // [B] Bloom calc
      sunBloom = exp(-distFromCenter * 1.5) * 2.0;
      sunBloom += exp(-distFromCenter * 2.5) * 1.5;
      sunBloom += exp(-distFromCenter * 0.7) * 0.8;
    ...
    // sRGB-like gamma approximation (applied to all paths, vanilla behavior)
    ...
    if (CUSTOM_SUN_INTENSITY > 0 && validLightData) {
      ...
      float edgeDist = length(uvOffset) * 2.0;
      float edgeFade = 1.0 - smoothstep(0.3, 0.65, edgeDist);
      r2.xyz = r2.xyz * edgeFade;
      
      // [F] 9x brightness boost
      r2.xyz = r2.xyz * 30.0;
      
      // [G] Corona add (faded at projection quad edges)
      float quadFade = 1.0 - smoothstep(0.3, 1.0, edgeDist);
      quadFade *= quadFade;  // Squared for smoother rolloff
      float3 coronaColor = float3(1.0, 0.9, 0.7) * sunCorona * quadFade;
      r2.xyz = r2.xyz + coronaColor;
      
      // [H] Bloom add (faded at projection quad edges)
      float3 bloomColor = float3(1.0, 0.95, 0.85) * sunBloom * quadFade;
