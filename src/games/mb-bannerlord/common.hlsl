...
// Bannerlord custom LUT sampling - non-standard 16x1 strip layout with dual-LUT blending
float3 SampleGameLUT(
  ...
  float3 sample0 = lut1.SampleLevel(lut_sampler, uv0, 0).xyz;
  float3 sample1 = lut1.SampleLevel(lut_sampler, uv1, 0).xyz;

  if (lut_blend > 0) {
    float3 lut2_sample0 = lut2.SampleLevel(lut_sampler, uv0, 0).xyz;
    float3 lut2_sample1 = lut2.SampleLevel(lut_sampler, uv1, 0).xyz;
    sample0 = lerp(sample0, lut2_sample0, lut_blend);
    sample1 = lerp(sample1, lut2_sample1, lut_blend);
  }

  return lerp(sample0, sample1, blue_frac);
}
...
LUTSampleResult LUTSAMPLE(
    ...
    float lut_blend,
    float3 untonemapped) {
  // Tonemap HDR input to SDR range for LUT sampling (sRGB LUT can't handle >1.0)
  float3 neutral_sdr = renodx::tonemap::renodrt::NeutralSDR(untonemapped);

  // Sample game LUT with SDR-range input, decode sRGB output to linear
  float3 lut_output_srgb = SampleGameLUT(lut_sampler, saturate(neutral_sdr), lut1, lut2, lut_blend);
  float3 graded_sdr = renodx::color::srgb::DecodeSafe(lut_output_srgb);

  // Upgrade: reconstruct HDR highlights using the tonemap ratio
