  ...
  out float4 o1 : SV_TARGET1)
{
  ...
  // Apply hue correction to fix pink->orange/yellow fire
  ...
  if (RENODX_TONE_MAP_TYPE != 0.f && shader_injection.tone_map_hue_correction > 0.f) {
    ...
    bool is_warm = (color.r > color.b * 1.2f) && (color.g > color.b * 0.8f);
    bool is_pink = color.r > color.g * 1.1f;  
    
    if (is_warm && is_pink) {
      
      float pink_amount = saturate((color.r - color.g) / max(color.r, 0.001f));
      float shift = pink_amount * strength;
      
      
      color.r = lerp(color.r, color.g * 1.1f, shift);
      color.g = lerp(color.g, color.g * 1.15f, shift * 0.5f);
    }
    
    r0.xyz = max(float3(0,0,0), color);
  }
