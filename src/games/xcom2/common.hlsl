...
// Builds a LUTSampleResult from the game's own LUT output (decoded to linear and scaled by max_channel to restore HDR range).
LUTSampleResult BuildLUTResult(float3 hdr_lut_linear, float3 linear_untonemapped) {
  LUTSampleResult result;
  result.graded = lerp(linear_untonemapped, hdr_lut_linear, shader_injection.color_grade_strength);
...
float3 HDRGRADE(LUTSampleResult lut_sample) {
  ...
  output = renodx::tonemap::neutwo::MaxChannel(
      output, shader_injection.peak_white_nits / shader_injection.diffuse_white_nits, 50.f);
  output = renodx::color::bt709::from::BT2020(output);
  return output;
}